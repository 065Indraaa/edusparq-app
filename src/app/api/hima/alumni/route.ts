...
export async function GET(req: NextRequest) {
  ...
  const items = await Alumni.find({ orgId }).sort({ createdAt: -1 }).lean();
...
export async function POST(req: NextRequest) {
  ...
  const { orgId, nama, tahunLulus, pekerjaan, perusahaan, posisi, kontak, linkedin, bersediaKonsultasi } = body;
  ...
  const item = await Alumni.create({
    ...
    kontak,
    linkedin,
    bersediaKonsultasi: !!bersediaKonsultasi,
...
export async function DELETE(req: NextRequest) {
  ...
  const existing = (await Alumni.findById(id).lean()) as { orgId: unknown } | null;
