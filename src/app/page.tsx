...
const features = [
  {
    icon: Bot,
    title: "Tutor AI Socratic",
    desc: "Bukan kasih jawaban instan — EduSparq menuntunmu berpikir lewat pertanyaan, biar konsep rumit benar-benar nempel.",
    tint: "text-primary bg-primary/10",
  },
  {
    icon: CalendarDays,
    title: "Manajemen Tenggat",
    desc: "Ekstrak deadline otomatis dari silabus & dokumen. Pengingat kontekstual biar nggak ada tugas yang kelewat.",
    tint: "text-amber-600 dark:text-amber-400 bg-amber-400/15",
  },
  {
    icon: PenTool,
    title: "Asisten Menulis",
    desc: "Bikin outline, parafrase, dan kelola sitasi akademik (APA, MLA, IEEE, Harvard) tanpa drama.",
    tint: "text-teal-600 dark:text-teal-400 bg-teal-400/15",
  },
  {
    icon: GraduationCap,
    title: "Persiapan Ujian",
    desc: "Prediksi soal UTS/UAS dari materimu sendiri, lengkap dengan flashcard untuk belajar aktif.",
    tint: "text-primary bg-primary/10",
  },
  {
    icon: Search,
    title: "Riset & Literatur",
    desc: "Cari jurnal dan paper relevan, ringkas abstrak, dan temukan versi akses terbuka yang legal.",
    tint: "text-teal-600 dark:text-teal-400 bg-teal-400/15",
  },
  {
    icon: Users,
    title: "Kolaborasi Kelompok",
    desc: "Workspace bareng buat tugas kelompok — bagi peran, share materi, dan pantau progres bersama.",
    tint: "text-amber-600 dark:text-amber-400 bg-amber-400/15",
  },
];
...
const steps = [
  {
    n: "01",
    title: "Unggah materimu",
    desc: "Masukkan slide, silabus, PDF, atau catatan kuliah. EduSparq langsung membacanya untukmu.",
  },
  {
    n: "02",
    title: "Tanya & kerjakan",
    desc: "Tanya tutor AI, ekstrak tenggat, susun tulisan, atau siapkan ujian — semua dari satu tempat.",
  },
  {
    n: "03",
    title: "Lihat sumbernya",
    desc: "Setiap jawaban dilengkapi tingkat kepercayaan dan rujukan, jadi kamu tahu seberapa bisa dipercaya.",
  },
];
  ...
  return (
    <div className="min-h-screen bg-background text-foreground overflow-x-hidden">
      ...
      <header className="sticky top-0 z-50 w-full border-b border-border/60 bg-background/80 backdrop-blur-xl">
        <div className="max-w-6xl mx-auto px-4 md:px-6 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <Link
              href="/login"
              className="hidden sm:inline-flex items-center px-4 h-10 rounded-xl text-sm font-semibold text-foreground hover:bg-muted transition-colors"
            >
              Masuk
            </Link>
            <Link
              href="/login"
              className="inline-flex items-center gap-1.5 px-4 h-10 rounded-xl text-sm font-bold bg-primary text-primary-foreground hover:bg-primary/90 transition-all shadow-sm active:scale-[0.98]"
            >
              Daftar gratis
              <ArrowRight size={15} />
            </Link>
      ...
      <section className="relative">
        ...
        <div className="max-w-6xl mx-auto px-4 md:px-6 pt-14 pb-16 md:pt-24 md:pb-24 grid lg:grid-cols-2 gap-12 lg:gap-8 items-center">
          {/* Left copy */}
          <motion.div variants={stagger} initial="hidden" animate="show" className="space-y-6 text-center lg:text-left">
            <motion.div variants={fadeUp} className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-semibold bg-primary/10 text-primary border border-primary/20">
              <Sparkles size={14} />
              Asisten akademik AI untuk mahasiswa Indonesia
            </motion.div>

            <motion.h1 variants={fadeUp} className="text-4xl md:text-5xl lg:text-6xl font-extrabold tracking-tight leading-[1.05]">
              Kuliah lebih cerdas,{" "}
              <span className="text-gradient">bukan lebih keras.</span>
            </motion.h1>

            <motion.p variants={fadeUp} className="text-base md:text-lg text-muted-foreground leading-relaxed max-w-xl mx-auto lg:mx-0">
              Dari nyatat materi sampai siap ujian — tugas, tenggat, tutor AI, riset, dan
              tulisan akademik ada di satu tempat. Dibuat khusus buat mahasiswa Indonesia.
            </motion.p>
            ...
            <motion.div variants={fadeUp} className="flex flex-wrap items-center gap-x-5 gap-y-2 justify-center lg:justify-start text-xs text-muted-foreground pt-2">
              {["Tanpa kartu kredit", "Bahasa Indonesia natural", "Hemat data"].map((t) => (
          ...
          >
            <div className="rounded-3xl border border-border bg-card shadow-xl p-5 space-y-4">
              ...
              <div className="flex justify-end">
                <div className="bg-primary text-primary-foreground rounded-2xl rounded-br-md px-4 py-2.5 text-sm max-w-[80%]">
                  Jelaskan inflasi pakai contoh harga kopi dong 🤔
                </div>
              </div>
              ...
              <div className="flex gap-2.5">
                ...
                <div className="space-y-3 flex-1">
                  <div className="bg-muted rounded-2xl rounded-tl-md px-4 py-3 text-sm leading-relaxed text-foreground border border-border">
                    Bayangin tahun lalu segelas kopi Rp18.000, sekarang Rp20.000.
                    Kenaikan harga umum dari waktu ke waktu itulah <strong>inflasi</strong> —
                    daya beli uangmu turun pelan-pelan.
                  </div>

                  {/* Confidence meter */}
                  <div className="space-y-1.5">
                    <div className="flex items-center justify-between">
                      <span className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">Tingkat kepercayaan</span>
                  ...
                  <div className="flex items-start gap-2 p-2.5 rounded-xl bg-primary/5 border-l-[3px] border-primary text-xs">
                    ...
                    <span className="text-muted-foreground">
                      <span className="font-semibold text-foreground">Pengantar Ekonomi Makro</span>, hal. 42 · materi kamu
                    </span>
                  </div>
                </div>
              </div>
            ...
            {/* floating badge */}
            <div className="absolute -bottom-4 -left-4 hidden sm:flex items-center gap-2 px-3 py-2 rounded-2xl bg-card border border-border shadow-lg">
              <ShieldCheck size={16} className="text-teal-500" />
              <span className="text-xs font-semibold">Selalu sebut sumbernya</span>
            </div>
          </motion.div>
        </div>
      </section>
      ...
      <section className="border-y border-border bg-muted/40">
        <div className="max-w-6xl mx-auto px-4 md:px-6 py-8 grid grid-cols-2 md:grid-cols-4 gap-6 text-center">
          {[
            { v: "6-in-1", l: "Tools akademik" },
            { v: "24/7", l: "Tutor AI siap" },
            { v: "4", l: "Format sitasi" },
      ...
      {/* ===== Features ===== */}
      <section id="fitur" className="max-w-6xl mx-auto px-4 md:px-6 py-16 md:py-24">
        <motion.div
          variants={fadeUp}
          initial="hidden"
          whileInView="show"
        ...
        >
          <span className="inline-flex items-center gap-1.5 text-xs font-bold uppercase tracking-wider text-teal-600 dark:text-teal-400">
            <Zap size={14} /> Satu app, semua kebutuhan kuliah
          </span>
          <h2 className="text-3xl md:text-4xl font-extrabold tracking-tight">
            Semua yang kamu butuhkan untuk kuliah
          </h2>
          <p className="text-muted-foreground">
            Berhenti lompat-lompat antar aplikasi. EduSparq menyatukan tugas, materi, riset, dan AI dalam satu alur kerja.
          </p>
        </motion.div>
          ...
          {features.map((f) => {
            ...
            return (
              <motion.div
                key={f.title}
                variants={fadeUp}
                className="group rounded-3xl border border-border bg-card p-6 hover:border-primary/40 hover:shadow-md hover:-translate-y-1 transition-all"
              >
      ...
      <section id="transparansi" className="bg-primary text-primary-foreground">
        <div className="max-w-6xl mx-auto px-4 md:px-6 py-16 md:py-24 grid lg:grid-cols-2 gap-12 items-center">
          <div className="space-y-5">
            <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold bg-white/10 border border-white/20">
              <ShieldCheck size={14} /> Yang bikin beda
            </span>
            <h2 className="text-3xl md:text-4xl font-extrabold tracking-tight">
              AI yang jujur soal seberapa yakin dia
            </h2>
            <p className="text-primary-foreground/80 leading-relaxed">
              Tiap jawaban faktual dilengkapi <strong>Confidence Meter</strong> dan rujukan ke sumber —
              entah itu dokumenmu sendiri, jurnal eksternal, atau inferensi AI. Kamu nggak perlu
              menebak mana yang bisa dipercaya.
            </p>
            <ul className="space-y-2.5 pt-2">
              {[
                "Tingkat kepercayaan: Tinggi, Sedang, Rendah, Tidak Diketahui",
                "Kutipan persis dari halaman sumbernya",
                "Peringatan jelas saat jawaban cuma inferensi",
              ].map((t) => (
          ...
          {/* meter card */}
          <div className="rounded-3xl bg-white/5 border border-white/15 backdrop-blur-sm p-6 space-y-4">
            {[
              { label: "Tinggi", filled: 4, color: "bg-teal-400", note: "Diverifikasi dari sumber kuat" },
      ...
      {/* ===== How it works ===== */}
      <section id="cara-kerja" className="max-w-6xl mx-auto px-4 md:px-6 py-16 md:py-24">
        <div className="text-center max-w-2xl mx-auto space-y-3 mb-12">
          <span className="inline-flex items-center gap-1.5 text-xs font-bold uppercase tracking-wider text-teal-600 dark:text-teal-400">
            <Smartphone size={14} /> Mulai dalam hitungan menit
          </span>
      ...
      <section className="max-w-6xl mx-auto px-4 md:px-6 pb-20">
        <div className="relative overflow-hidden rounded-[2rem] border border-border bg-card p-8 md:p-14 text-center">
          ...
          <h2 className="text-3xl md:text-4xl font-extrabold tracking-tight max-w-2xl mx-auto">
            Siap kuliah lebih efisien semester ini?
          </h2>
          <p className="text-muted-foreground mt-3 max-w-lg mx-auto">
            Gabung gratis dan rasakan bedanya punya asisten akademik yang ngerti kebutuhan mahasiswa Indonesia.
          </p>
          <Link
            href="/login"
            className="inline-flex items-center justify-center gap-2 px-7 h-12 mt-7 rounded-xl bg-primary text-primary-foreground font-bold hover:bg-primary/90 transition-all shadow-sm hover:shadow-md active:scale-[0.98]"
          >
